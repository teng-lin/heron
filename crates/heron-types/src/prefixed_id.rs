...
//!
//! Lives in `heron-types` so every consumer crate (`heron-bot`,
//! `heron-policy`, `heron-realtime`) instantiates the macro from one
//! source rather than duplicating it per crate.
//!
//! Each ID kind (`BotId`, `PersonaId`, `MeetingId`, `UtteranceId`,
//! `VoiceId`, `SessionId`, `ResponseId`, …) wraps a UUID plus a
//! compile-time prefix string. The wire form is `prefix_<uuid>`
...
/// # Invocation
///
/// Always invoke as `heron_types::prefixed_id! { ... }`. The macro
/// is not `pub use`-exportable: Rust's macro resolution doesn't
/// follow `pub use` re-exports, so a downstream crate that does
/// `pub use heron_types::prefixed_id;` would shadow the *module*
/// without making the macro callable from elsewhere.
///
...
macro_rules! prefixed_id {
    ($(#[$attr:meta])* $vis:vis $name:ident, $prefix:literal) => {
        ...
        impl ::std::str::FromStr for $name {
            type Err = $crate::prefixed_id::IdParseError;
            fn from_str(s: &str) -> ::std::result::Result<Self, Self::Err> {
                $crate::prefixed_id::parse_prefixed(Self::PREFIX, s).map(Self)
            }
        }
