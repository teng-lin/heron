...
//! ## Eviction policy
//!
//! On overflow we drop the **newest** existing entry (highest
//! `captured_at_micros`) and keep the just-inserted frame. This
//! biases toward retaining older audio, which is closer to what
//! the consumer pops next — the alternative (drop the new one)
//! would refuse a frame that may already be the oldest in the
//! buffer. The cost is dropping audio further out in the future,
//! which the upstream pipeline will retry-send or reconstruct from
//! the backend's own jitter handling.
//!
//! ## Late-drop policy
//!
...
pub enum InsertOutcome {
    ...
    DroppedDuplicate,
    /// Buffer was full. The newest existing frame was evicted to
    /// make room and the just-inserted frame was stored. Returned
    /// so the bridge can log the overflow.
    Overflow,
}
...
impl JitterBuffer {
    ...
    pub fn insert(&mut self, frame: PcmFrame) -> InsertOutcome {
        ...
        // Advance watermark before the overflow branch so an
        // overflow insert still pushes the late-drop line forward.
        ...
        if self.config.late_drop_micros > 0 {
            let candidate = frame
                .captured_at_micros
                .saturating_sub(self.config.late_drop_micros);
            if candidate > self.watermark_micros {
                self.watermark_micros = candidate;
            }
        }

        // Overflow branch: evict the newest existing key first, then
        // insert the new frame. We check `>=` rather than `>` because
        // we're about to add one — `len == max` means the post-insert
        // length would exceed the cap.
        if self.frames.len() >= self.config.max_frames {
            // `max_frames == 0` is a degenerate config but we
            // shouldn't panic on it; pop_last returns None and we
            // skip the eviction. The frame still gets stored below
            // and we report Overflow so the caller sees that the
            // configured cap is unusable.
            self.frames.pop_last();
            self.frames.insert(frame.captured_at_micros, frame);
